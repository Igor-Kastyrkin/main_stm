...
String currSpeed = "S0";
String currAccel = "A0";
String currStpDpth = "K28";
String currStpAngl = "K20";
String currRbStps  = "d75";
...
actions Actions  = walkFf;
actions oldActions = Actions;

short maxStepsCount = 0;
...
void setup()
{
  ...
  float telegaSpeed = 9200.0;
  float telegaAccel = 10000.0; //8000
...
//      Actions = turnL;
      ...
      delay(10);
      Serial1.println(String(rbOrient));
      delay(10);
...
byte walkForward(posOfMotors & mot)
{
/*  if (stWork == StRec)
  {
    Actions = wait;
  ...
  else if (standStill()) // стоим на обеих ногах?
  { // если левая впереди, то телега справа и надо идти вперед, то много лишних действий.
    ...
    while (1)
    {
      ...
      if (fBreak(not_leg, foot, mot))
      {
        fSendState(stWork, Actions, halfLengthStepsCount/*, mot*/);
        return 1;
      }
      if ((stWork == StPlay) && (halfLengthStepsCount >= maxStepsCount))
      {
...
byte walkBackward(posOfMotors & mot)
{
  /*
...
void fCalibration(UART_Serial & lSer, UART_Serial & rSer, posOfMotors & mot)
{
  ...
  lSer.prepareMessage( 'h', LeftLegCurrentSteps); // втягиваем
  rSer.prepareMessage( 'h', RightLegCurrentSteps); // втягиваем
...
bool fBreak(robot_leg leg, MKmotor Uzel, posOfMotors & mot)
{
...
#endif
  {
    ...
    if (RF_data[0] == 'R') // команда повернуть вправо?
    {
      rbOrient -= x;
      //  rotPlace(mot, minRbtStep, x);
      Actions = turnR;
    ...
    if (RF_data[0] == 'L') // команда повернуть влево
    {
      rbOrient += x;
      Actions = turnL;
      fSendState(stWork,  Actions, x);
...
*/
	fAddInActionInRecordMode(standStill);
  }
}

// Отправка текущего состояния для записи и воспроизведения
void fSendState(StadyWork WorkSt, actions Action, unsigned short halfLengthStepsCount)
{
  ...
  if ((WorkSt == StRec))
  {
    switch (Action)
    {
      case walkFf:
        ...
        //        delay(20);
        Str1 = "s";
        Str2 = String(halfLengthStepsCount); // число шагов конвертируем в строку
        ...
        Str1.toCharArray(Str3, Str1.length() + 1); // конвертируем из String в *char и сохраняем в Str3
        radio.write(Str3, Str1.length() + 1);    // отправляем сколько шагов прошли прошли
        Stp.toCharArray(Str3, Stp.length() + 1);
        radio.write(Str3, Stp.length() + 1);     // отправляем команду
...
#ifdef _NRF24_
        radio.stopListening();
        Str1.toCharArray(Str3, Str1.length() + 1);
        radio.write(Str3, Str1.length() + 1);
        Stp.toCharArray(Str3, Stp.length() + 1);
        radio.write(Str3, Stp.length() + 1);
        halfLengthStepsCount = 0;
        radio.startListening();
#else
        /*
...
#ifdef _EPROM1_
        Str1 = "L";
        Str2 = String(halfLengthStepsCount); // конвертируем в строку
        Str1.concat(Str2);
        writeString(Str1);
        //        fOtladkaMes(readString(StrAddr - incriment));
...
#ifdef _NRF24_
    radio.stopListening();
    Str1.toCharArray(Str3, Str1.length());
    radio.write(Str3, Str1.length());
    //        radio.write(&Stp, sizeof(char));
    radio.startListening();
    /*
      #else
      #ifdef SERIAL_USB
...
bool orient_steps(robot_leg leg, step_dir dir, posOfMotors & mot)
{
  ...
  if (leg == left_leg)
  {
    if (readyForRotLeftFoot())   // стоим на левой ноге?
    {
      ...
      angleR =  rbOrient * calc_angle(1, m_stp) * 2 - stepAngle;
      angleL = -rbOrient * calc_angle(1, m_stp) * 2 + stepAngle;
      leed_leg = leftLeed;
  ...
  else if (leg == right_leg)   // стоим на правой ноге?
  {
    ...
    {
      angleR =  rbOrient * calc_angle(1, m_stp) * 2 + stepAngle; // куда нужно повернуть правую
      angleL = -rbOrient * calc_angle(1, m_stp) * 2 - stepAngle; // куда нужно повернуть левую
      leed_leg = rightLeed;
...
byte rotPlace(posOfMotors & mot, short minStep, short newOrient)
{
  ...
  for (i = 0; abs(lastPreposition) > abs(i); i += minStep)
  {
    ...
    {
      // поворачиваемся вперед на правой ноге на нужный угол
      if (orient_steps(right_leg, dir, mot) && (stWork == StWork)) break;
    ...
    {
      // поворачиваемся на левой ноге
      if (orient_steps(left_leg, dir, mot) && (stWork == StWork)) break;
      //	  change_orient(turn_dir, mot, minStep);
      // опускаем правую ногу
    ...
    } else if (standStill())  // стоим на обоих ногах
    {
      if (pr_telega.dir_flg  ==  right)  {
  ...
  if (rbOrient != (old_Orient + newOrient))
  {
    ...
    {
      // поворачиваемся вперед на правой ноге на нужный угол
      orient_steps(right_leg, dir, mot);
...
void topLeft(posOfMotors & mot)
{
  ...
  if (standStill()) pr_telega.RotateStpsOnly(*motorLink, telega, pr_telega.DriveRight());
...
void topRight(posOfMotors & mot)
{
  ...
  if (standStill()) pr_telega.RotateStpsOnly(*motorLink, telega, pr_telega.DriveLeft());
  ...
  if (leg == left_leg)
  {
    if (readyForRotRightFoot() ) // стоим на правой ноге?
    {
      if (direction == 0) // вращать в разные стороны по очереди
      {
        while (times > 0)
        {
          SerL.prepareMessage( 'c', rbOrient * calc_angle(1, m_stp) * 2 - stepAngle);
  ...
  if (leg == right_leg)
  {
    if (readyForRotLeftFoot() ) // стоим на левой ноге?
    {
      if (direction == 0) // вращать в разные стороны по очереди
      {
        while (times > 0)
        {
          SerR.prepareMessage( 'c', rbOrient * calc_angle(1, m_stp) * 2 - stepAngle);
...
byte fShake(posOfMotors & mot, const byte mode)
{
  static bool HPos;
  if (!mode) HPos == 1 ? HPos = 0 : HPos = 1;
  else if (mode == 1)
  {
    LeftLegCurrentSteps = CurrentZero;
    RightLegCurrentSteps = CurrentZero;
...
void fRoll(posOfMotors & mot)
{
	...
	orient_steps(right_leg, forward, mot);
...
//  иначе 0.
bool fEastOrWestStandingCalc(void)
{
